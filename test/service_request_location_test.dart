...
void main() {
  ...
  test('API repository sends text and real media bytes as multipart', () async {
    ...
    expect(result.requestId, '12');
    expect(request.headers['Authorization'], 'Bearer test-token');
    expect(request.fields['category'], '12');
    expect(request.fields['description'], 'My bike is not starting.');
    expect(request.fields['latitude'], '27.671234');
    expect(request.fields['longitude'], '85.339876');
    ...
    await expectLater(
      ...
      throwsA(
        isA<ServiceRequestException>().having(
          (error) => error.message,
          'message',
          contains('description: This field may not be blank.'),
        ),
      ),
    );
  });
...
class _ValidationErrorClient extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    final body = jsonEncode({
      ...
      'category': ['Invalid pk "9999" - object does not exist.']
    });
    return http.StreamedResponse(
      Stream.value(utf8.encode(body)),
      400,
