  ...
  Future<List<Skill>> getSkills() async {  //It returns the list of skill obj in comming future
    ...
    if (response.statusCode == 200) {
      ...
      return (data["skills"] as List) //returns the value of skill(which is as list) that was send in response . "skills": [ {...}, {...}, {...} ]
          .map((skill) => Skill.fromJson(skill)) // .map() calls Skill.fromJson on each item in this list. and converts into dart objects. The item is temporarily stored in the variable skill in each iteration. Returns the output into Skill
